...
@section('page_title', $dis->title)
@section('header__title', $dis->title)

@section('bg')
<style>
    header {
        @if(mb_strlen($dis->title)>40) padding: 500px !important;
        @elseif(mb_strlen($dis->title)>30) padding: 470px !important;
        @else padding: 100px @endif height: max-content !important;
        background-size: cover;
        background: url("{{asset('assets/front/images/newbuildings/'.$dis->img)}}") center center/cover no-repeat
    }
</style>
@endsection
